...
from sarlopt.networks.lstm_td3_critic_network import (
    LSTMTD3CriticNetwork,
    LSTMTD3InputCritic,
)
from sarlopt.policies.lstm_td3_policies import LSTMTD3ActorPolicy, LSTMTD3GaussianPolicy
...
class LSTMTD3Agent(tf_agent.TFAgent):
    ...
    def __init__(
        ...
        critic_input_spec = LSTMTD3InputCritic(
            ...
            actions=tensor_spec.TensorSpec([n_elements_actions], dtype=tf.float32),
        )
        ...
        self._target_critic_network_1 = common.maybe_copy_target_network_with_checks(
            self._critic_network_1, None, "TargetCriticNetwork1"
        )
        ...
        self._target_critic_network_2 = common.maybe_copy_target_network_with_checks(
            self._critic_network_2, None, "TargetCriticNetwork2"
        )
    ...
    def _train(self, experience, weights=None):
        ...
        with tf.GradientTape(watch_accessed_variables=False) as tape:
            ...
            actor_loss = self.actor_loss(
                o_t, hist_o_t, hist_a_t, weights=weights, training=True
            )
        ...
        return tf_agent.LossInfo(total_loss, LSTMTD3Info(actor_loss, critic_loss))
    ...
    def critic_loss(
        ...
        with tf.name_scope("critic_loss"):
            ...
            target_actions, _ = self._target_actor_network(
                LSTMTD3InputActor(history=hist_t1, observations=o_t1),
                None,
                training=training,
            )
            ...
            target_q_values_1, _ = self._target_critic_network_1(
                LSTMTD3InputCritic(
                    history=hist_t1, actions=noisy_target_actions, observations=o_t1
                ),
                None,
                training=False,
            )
            ...
            td_targets = tf.stop_gradient(
                self._reward_scale_factor * r_t1 + self._gamma * d_t1 * target_q_values
            )

            pred_td_targets_1, _ = self._critic_network_1(
                LSTMTD3InputCritic(history=hist_t, observations=o_t, actions=a_t),
                None,
                training=training,
            )
    ...
    def actor_loss(
        ...
        with tf.name_scope("actor_loss"):
            ...
            actions, _ = self._actor_network(
                LSTMTD3InputActor(history=hist_t, observations=o_t),
                None,
                training=training,
            )

            q_values, _ = self._critic_network_1(
                LSTMTD3InputCritic(history=hist_t, actions=actions, observations=o_t),
                None,
                training=False,
            )
            actor_loss = -q_values
