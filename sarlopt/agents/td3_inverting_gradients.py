...
@gin.configurable
class Td3AgentInvertingGradients(tf_agent.TFAgent):
    """A TD3 Inverting Gradients Agent."""
    ...
    def __init__(
        ...
        exp_noise_std: types.Float = 0.1,
        exp_noise_std_end: Optional[types.Float] = None,
        exp_noise_steps: Optional[types.Int] = None,
        ...
        Args:
          ...
          exp_noise_std: Initial scale factor on exploration policy noise.
          exp_noise_std_end: Final scale factor on exploration policy noise.
          exp_noise_steps: Number of steps until the exploration policy noise factor
            reaches the exp_noise_std_end.
        ...
        self._target_critic_network_1 = common.maybe_copy_target_network_with_checks(
            self._critic_network_1, target_critic_network, "TargetCriticNetwork1"
        )
        ...
        self._target_critic_network_2 = common.maybe_copy_target_network_with_checks(
            self._critic_network_2, target_critic_network_2, "TargetCriticNetwork2"
        )

        self._high_obs = tf.constant(
            self._critic_network_1.input_tensor_spec[0].maximum, dtype=tf.float32
        )
        self._low_obs = tf.constant(
            self._critic_network_1.input_tensor_spec[0].minimum, dtype=tf.float32
        )

        self._actor_optimizer = actor_optimizer
        self._critic_optimizer = critic_optimizer

        self._exp_noise_std = exp_noise_std
        self._exp_noise_std_end = exp_noise_std_end
        self._exp_noise_steps = exp_noise_steps
        ...
        if self._exp_noise_steps is None or self._exp_noise_std_end is None:
            collect_policy = gaussian_policy.GaussianPolicy(
                collect_policy, scale=self._exp_noise_std, clip=True
            )
        else:
            collect_policy = gaussian_policy_linear_decay.GaussianPolicyLinearDecay(
                collect_policy,
                self._exp_noise_std,
    ...
    def _initialize(self):
        ...
        common.soft_variables_update(
            self._actor_network.variables, self._target_actor_network.variables, tau=1.0
        )

    def _get_target_updater(
        self, tau: Optional[types.Float] = 1.0, period: Optional[types.Int] = 1
    ):
        ...
        Returns:
          periodically_update: A callable that performs a soft update
    ...
    def _train(self, experience, weights=None):
        ...
        return tf_agent.LossInfo(total_loss, td3_agent.Td3Info(actor_loss, critic_loss))
    ...
    def critic_loss(
        ...
        with tf.name_scope("critic_loss"):
            ...
            noisy_target_actions = tf.nest.map_structure(
                add_noise_to_action, target_actions
            )
            ...
            critic_loss = self._td_errors_loss_fn(
                td_targets, pred_td_targets_1
    ...
    def actor_loss(
        ...
        with tf.name_scope("actor_loss"):
            ...
            q_values, _ = self._critic_network_1(
                (time_steps.observation, actions), time_steps.step_type, training=False
            )
            actor_loss = -q_values
            # Sum over the time dimension.
            ...
            actor_loss = common.aggregate_losses(
                per_example_loss=actor_loss, sample_weight=weights
            ).total_loss
    ...
    def actor_grads(
        ...
        """Computes the actor_grads for TD3 training (Applies
        the Inverting Gradient technique).

            Args:
              time_steps: A batch of timesteps.
              weights: Optional scalar or element-wise (per-batch-entry) importance
                weights.
              training: Whether the grad is being used for training.
            Returns:
              actor_grads: Gradients of the actor network.
        """
        with tf.name_scope("actor_grads"):
            ...
            with tf.GradientTape(watch_accessed_variables=False) as outer_tape:
                ...
                outer_tape.watch(trainable_actor_variables)
                actions, _ = self._actor_network(
                    time_steps.observation, time_steps.step_type, training=training
                )
                with tf.GradientTape(watch_accessed_variables=False) as inner_tape:
                    inner_tape.watch(actions)
                    q_values, _ = self._critic_network_1(
                        (time_steps.observation, actions),
                        time_steps.step_type,
                        training=False,
                    )
                dq_da = inner_tape.gradient(q_values, actions, output_gradients=weights)
                mul_consts = tf.where(
                    tf.math.greater(dq_da, tf.constant([0.0], dtype=tf.float32)),
                    actions_upper_bounds - actions,
                    actions - actions_lower_bounds,
                )
                mul_consts = tf.divide(mul_consts, actions_bounds_delta)
                adjusted_dq_da = tf.math.multiply(dq_da, mul_consts)
            da_dmu = outer_tape.gradient(
                actions, trainable_actor_variables, output_gradients=adjusted_dq_da
            )
            da_dmu = [
                tf.math.divide(tf.math.negative(grad), batch_size) for grad in da_dmu
            ]

            for grad_tensor in da_dmu:
                tf.debugging.check_numerics(grad_tensor, "Actor grad is inf or nan.")
