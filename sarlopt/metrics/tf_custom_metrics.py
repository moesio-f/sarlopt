...
class AverageBestObjectiveValueMetric(tf_metric.TFStepMetric):
    def __init__(
        ...
        super(AverageBestObjectiveValueMetric, self).__init__(name=name, prefix=prefix)
        self._function = common.function(function)
        self._buffer = tf_metrics.TFDeque(buffer_size, dtype)
        self._dtype = dtype
        self._best_value = common.create_variable(
            initial_value=0, dtype=dtype, shape=(), name="BestValue"
        )
        self._current_value = common.create_variable(
            initial_value=0, dtype=dtype, shape=(), name="CurrentValue"
        )
    ...
    def call(self, trajectory):
        ...
        if trajectory.is_first():
            ...
            if tf.reduce_all(tf.math.less(self._current_value, self._best_value)):
                self._best_value.assign(self._current_value)
...
class ConvergenceMultiMetric(tf_metric.TFMultiMetricStepMetric):
    def __init__(
        ...
        self._buffer = tf_metrics.TFDeque(buffer_size, dtype, shape=(trajectory_size,))
        self._buffer_single_trajectory = tf_metrics.TFDeque(
            trajectory_size, dtype, shape=(1,)
        )
        ...
        super(ConvergenceMultiMetric, self).__init__(
            name=name, prefix=prefix, metric_names=self._metric_names
        )
    ...
    def result(self):
        return [self._buffer.mean(), self._buffer.min(), self._buffer.max()]
