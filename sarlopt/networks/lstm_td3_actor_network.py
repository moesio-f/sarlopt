...
class LSTMTD3ActorNetwork(network.Network):
    ...
    def __init__(
        ...
        memory_state_spec = tf.nest.map_structure(
            functools.partial(
                ...
                dtype=tf.float32,
                name="network_memory_state_spec",
            ),
        ...
        super(LSTMTD3ActorNetwork, self).__init__(
            input_tensor_spec=input_tensor_spec, state_spec=memory_state_spec, name=name
        )
    ...
    def call(self, inputs, step_type, network_state=(), training=False):
        ...
        extracted_memory, network_state = self._dynamic_unroll(
            extracted_memory, initial_state=network_state, training=training
        )
