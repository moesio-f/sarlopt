...
class TFFunctionEnvV3(tf_environment.TFEnvironment):
    ...
    def __init__(
        ...
        if self._use_curriculum:
            ...
            self._curriculum_cls = tf.constant(
                [cls for (_, cls) in curriculum_strategy], dtype=tf.int32
            )
            self._curriculum_eps = tf.constant(
                [eps for (eps, _) in curriculum_strategy], dtype=tf.int32
            )
        ...
        if action_bounds is not None:
            ...
            action_spec = specs.TensorSpec(
                shape=tf.TensorShape([self._dims]), dtype=tf.float32, name="action"
            )
        # 3 * dims: [grad_mag_dims] + [grad_sign_dims] + [velocity_dim]
        observation_spec = specs.TensorSpec(
            shape=tf.TensorShape([3 * self._dims]), dtype=tf.float32, name="observation"
        )
        ...
        self._episode_ended = common.create_variable(
            name="episode_ended", initial_value=False, dtype=tf.bool
        )
        self._total_episodes = common.create_variable(
            name="total_episodes", initial_value=0, dtype=tf.int32
        )
        self._steps_taken = common.create_variable(
            name="steps_taken", initial_value=0, dtype=tf.int32
        )
        self._duration = tf.constant(value=duration, dtype=tf.int32, name="duration")
        ...
        # States
        self._x0 = common.create_variable(
            name="x0",
            shape=tf.TensorShape([self._dims]),
            initial_value=0,
            dtype=tf.float32,
        )
        self._x = common.create_variable(
            name="x",
            shape=tf.TensorShape([self._dims]),
            initial_value=0,
            dtype=tf.float32,
        )
        self._fx = common.create_variable(
            name="fx", shape=(), initial_value=0, dtype=tf.float32
        )
        ...
        def log_processing(g: tf.Tensor):
            abs_g = tf.abs(g)
            return tf.cond(
                pred=tf.greater_equal(abs_g, tf.math.exp(-self._p)),
                true_fn=lambda: tf.math.divide(tf.math.log(abs_g), self._p),
                false_fn=lambda: tf.constant(-1.0, dtype=tf.float32),
            )

        def sign_processing(g: tf.Tensor):
            return tf.cond(
                pred=tf.greater_equal(tf.abs(g), tf.math.exp(-self._p)),
                true_fn=lambda: tf.sign(g),
                false_fn=lambda: tf.multiply(g, tf.math.exp(-self._p)),
            )
        ...
        self._c = tf.constant(
            tf.math.reciprocal(tf.math.log(self._b)), dtype=tf.float32
        )

        def bi_symmetrical_log(fx):
            log_10 = tf.divide(
                tf.math.log(tf.math.add(tf.abs(fx / self._c), 1)), tf.math.log(self._b)
            )
    ...
    def _current_time_step(self) -> ts.TimeStep:
        ...
        with tf.control_dependencies([grads]):
            ...
            tf.debugging.check_numerics(masked_grads, message="masked_grads is NaN/Inf")
        ...
        with tf.control_dependencies([log_grad, sign_grad, avg_velocity]):
            observation = tf.concat(
                [log_grad, sign_grad, avg_velocity], axis=-1, name="observation_concat"
            )
        ...
        def mid():
            return (
                ...
                tf.reshape(-self._reward_transformation_fn(fx), shape=()),
            )
    ...
    def _reset(self) -> ts.TimeStep:
        ...
        with tf.control_dependencies([reset_ended, reset_steps, current_domain]):
            ...
            with tf.control_dependencies([x_reset, x0_reset]):
                ...
                with tf.control_dependencies([fx, grad]):
                    ...
                    with tf.control_dependencies([fx_reset, grads_x_reset]):
    ...
    def _step(self, action):
        ...
        def take_step():
            ...
            with tf.control_dependencies(
                [x_update, fx_update, grads_update, steps_update, episode_finished]
            ):
        ...
        return tf.cond(
            pred=tf.math.reduce_any(self._episode_ended),
            true_fn=reset_env,
            false_fn=take_step,
        )
