...
class TFFunctionEnvV3Test(tf.test.TestCase):
    ...
    def test_run_episode(self):
        ...
        action = tf.expand_dims(
            tf.repeat(tf.constant(1.0, dtype=tf.float32), repeats=(self.dims,)), axis=0
        )
        ...
        log_abs_grads = tf.constant([[0.62076, 0.636727]], dtype=tf.float32)
        sign_grads = tf.constant([[1.0, 1.0]], dtype=tf.float32)
        final_observation = tf.concat([log_abs_grads, sign_grads, action], axis=-1)
