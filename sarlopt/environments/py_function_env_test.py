...
class PyFunctionEnvTest(test_utils.TestCase):
    ...
    def setUp(self):
        ...
        self.env = PyFunctionEnv(function=self.function, dims=self.dims, seed=self.seed)
    ...
    def test_validate_rewards(self):
        states = [
            np.zeros((self.dims,)).astype(np.float32),
        ...
        for action in actions:
            for state in states:
                ...
                self.assertEqual(-self.function(state), ts.reward)
    ...
    def test_validate_state_clip(self):
        ...
        for m in [1.0, -1.0]:
            ...
            self.assertAllEqual(
                m * next_state, self.env.current_time_step().observation
            )
    ...
    def test_validate_episode_end(self):
        ...
        self.assertEqual(PyFunctionEnv.MAX_STEPS, steps)
