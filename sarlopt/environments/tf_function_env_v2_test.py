...
class TFFunctionEnvV2Test(tf.test.TestCase):
    def setUp(self):
        ...
        self.functions = [self.function] + rand_fn.random_shifted_functions(
    ...
    def test_validate_state_clip(self):
        ...
        action = tf.expand_dims(
            tf.repeat(tf.constant([500.0], dtype=tf.float32), repeats=(self.dims,)),
            axis=0,
        )
        env.step(action)
        time_step = env.current_time_step()
        ...
        self.assertAllEqual(
            tf.expand_dims(
                tf.repeat(tf.constant([100.0], dtype=tf.float32), repeats=(self.dims,)),
                axis=0,
            ),
    ...
    def testRunEpisode(self):
        ...
        action = tf.expand_dims(
            tf.repeat(tf.constant([1.0], dtype=tf.float32), repeats=(self.dims,)),
            axis=0,
        )
        ...
        final_state = tf.clip_by_value(
            initial_state + self.duration * action,
