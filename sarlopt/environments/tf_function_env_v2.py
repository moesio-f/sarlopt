...
class TFFunctionEnvV2(tf_environment.TFEnvironment):
    ...
    def __init__(
        ...
        self._fn_index = common.create_variable(
            name="fn_index", initial_value=0, dtype=tf.int32
        )

        self._fn_evaluator = lambda x: tf.nest.map_structure(
            lambda f: lambda: f(x), self._functions
        )

        self._domain_min = tf.cast(self._functions[0].domain.min, tf.float32)
        self._domain_max = tf.cast(self._functions[0].domain.max, tf.float32)
        self._dims = dims
        ...
        self._fn_index.assign(
            value=self._rng.uniform(
                shape=(), minval=0, maxval=self._n_functions, dtype=tf.int32
            )
        )
    ...
    def _current_time_step(self) -> ts.TimeStep:
        ...
        def mid():
            return (
                ...
                tf.reshape(
                    tf.math.negative(tf.switch_case(fn_index, branches)), shape=()
                ),
            )
    ...
    def _step(self, action):
        ...
        def take_step():
            ...
            with tf.control_dependencies(
                [state_update, steps_update, episode_finished]
            ):
