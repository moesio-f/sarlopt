...
class ReverbLearnerPER(learner.Learner):
    ...
    def __init__(
        ...
        self._initial_is_weight_exp = tf.constant(
            initial_is_weight_exp, dtype=tf.float32
        )
        ...
        self._is_weight_exp = tf.Variable(
            self._initial_is_weight_exp, dtype=tf.float32, name="is_weight_exp"
        )
        ...
        def update_priorities(sample, loss):
            ...
            priorities = tf.clip_by_value(
                tf.math.abs(td_errors),
                clip_value_min=priority_clip_min,
                clip_value_max=priority_clip_max,
            )
            reverb_replay_buffer.update_priorities(
                sample_info.key[:, 0], tf.cast(priorities, tf.float64)
            )
    ...
    def single_train_step(self, iterator):
        ...
        if self.use_kwargs_in_agent_train:
            loss_info = self.strategy.run(
                self._agent.train,
                kwargs=dict(experience=experience, weights=is_weights),
            )
            self.strategy.run(
            ...
            loss_info = self.strategy.run(
                self._agent.train, args=(experience, is_weights)
            )
            self.strategy.run(
                self.after_train_strategy_step_fn,
    ...
    def _get_is_weights(self, sample_info):
        ...
        weights = tf.math.pow(
            tf.multiply(size, probs), tf.math.negative(self._is_weight_exp)
        )
        weights = tf.math.divide(weights, tf.reduce_max(weights))

        self._is_weight_exp.assign(
            tf.clip_by_value(
                self._is_weight_exp + self._is_weight_exp_update,
