...
class TFLSTMTD3ReplayBuffer(tf_uniform_replay_buffer.TFUniformReplayBuffer):
    ...
    def _num_episodes(self):
        return tf.cast(
            tf.math.floordiv(self._get_last_id() + 1, self._episode_length), tf.int64
        )

    def _num_partial_episodes(self):
        return tf.cond(
            tf.not_equal(tf.math.mod(self._get_last_id() + 1, self._episode_length), 0),
    ...
    def _get_next(self, sample_batch_size=None, num_steps=None, time_stacked=True):
        with tf.device(self._device), tf.name_scope(self._scope):
            with tf.name_scope("get_next"):
                ...
                assert_time_stacked = tf.debugging.Assert(
                    tf.equal(time_stacked, True), [time_stacked]
                )
                ...
                with tf.control_dependencies([assert_nonempty, assert_time_stacked]):
                    if num_steps is None:
                        ...
                        with tf.control_dependencies([assert_valid_steps]):
                            ...
                            id_after_complete_episodes = tf.multiply(
                                n_complete_episodes, self._episode_length
                            )
                            ...
                            assert_exists_ids = tf.debugging.Assert(
                                tf.math.logical_or(
                                    tf.math.logical_and(
                                        tf.greater_equal(
                                            self._episode_length, num_steps
                                        ),
                            ...
                            with tf.control_dependencies([assert_exists_ids]):
                                ...
                                probability = tf.cond(
                                    ...
                                    true_fn=lambda: 0.0,
                                    false_fn=lambda: 1.0
                                    / tf.cast(num_ids * self._batch_size, tf.float32),
                                )
                                ...
                                def history_offset(id_):
                                    ...
                                    limits = tf.cond(
                                        tf.greater_equal(
                                            id_, id_after_complete_episodes
                                        ),
                                        true_fn=lambda: tf.stack(
                                            [id_after_complete_episodes, last_id + 1],
                                            axis=0,
                                        ),
                                    ...
                                    offset_0_start = tf.math.maximum(
                                        limits[0], id_1 - num_steps
                                    )
                                    offset_0 = tf.math.subtract(
                                        tf.range(
                                            start=offset_0_start,
                                            limit=id_1,
                                            dtype=tf.int64,
                                        ),
                                        id_,
                                    ...
                                    offset_1_limit = tf.math.minimum(
                                        limits[1], id_1 + remaining_steps
                                    )
                                ...
                                if sample_batch_size:
                                    # [B, num_steps]
                                    ids = tf.tile(
                                        tf.expand_dims(ids, -1), [1, num_steps]
                                    )
                ...
                buffer_info = tf_uniform_replay_buffer.BufferInfo(
                    ids=data_ids, probabilities=probabilities
                )
