...
class CustomNormal(distribution.Distribution):
    ...
    def __init__(
        self, loc, scale, validate_args=False, allow_nan_stats=True, name="Normal"
    ):
    ...
    def loc(self, value):
        dtype = dtype_util.common_dtype(
            [self.loc, self.scale, value], dtype_hint=tf.float32
        )
    ...
    def scale(self, value):
        dtype = dtype_util.common_dtype(
            [self.scale, self.loc, value], dtype_hint=tf.float32
        )
    ...
    def _log_prob(self, x):
        ...
        log_normalization = tf.constant(
            0.5 * np.log(2.0 * np.pi), dtype=self.dtype
        ) + tf.math.log(scale)
...
def _kl_normal_normal(a, b, name=None):
    ...
    with tf.name_scope(name or "kl_normal_normal"):
        ...
        return (
            ...
            + 0.5 * tf.math.expm1(2.0 * diff_log_scale)
            - diff_log_scale
        )
