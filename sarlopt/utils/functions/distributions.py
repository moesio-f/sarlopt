...
class UniformFunctionDistribution(FunctionDistribution):
    def __init__(
        ...
        self._fn_evaluator = lambda x: tf.nest.map_structure(
            lambda f: lambda: f(x), self._fns
        )

        self._fns_names = tf.constant([f.name for f in self._fns], dtype=tf.string)
        self._domains = tf.constant(
            [(f.domain.min, f.domain.max) for f in self._fns], dtype=dtype
        )
        self._len_classes = tf.constant(
            [len(fn_l) for fn_l in functions], dtype=tf.int32
        )
        ...
        def mapper(cls_index, fn_index):
            start_flat_index = tf.cond(
                tf.greater(cls_index, 0),
                lambda: tf.gather(self._cum_sum_len_classes, cls_index - 1),
                lambda: 0,
            )
        ...
        self._class_index = tf.Variable(
            name="class_index", initial_value=0, trainable=False, dtype=tf.int32
        )
        self._fn_index = tf.Variable(
            name="fn_index", initial_value=0, trainable=False, dtype=tf.int32
        )

        self._hshift = tf.Variable(
            name="hshift",
            initial_value=tf.zeros(shape=(self._dims_params,), dtype=dtype),
            trainable=False,
            dtype=dtype,
        )
        self._vshift = tf.Variable(
            name="vshift", initial_value=0, trainable=False, dtype=dtype
        )
        self._scale = tf.Variable(
            name="scale", initial_value=0, trainable=False, dtype=dtype
        )
    ...
    def sample(self) -> None:
        ...
        random_scale = self._rng.uniform(
            shape=(), minval=self._scale_min, maxval=self._scale_max, dtype=self._dtype
        )
        random_cls_index = self._rng.uniform(
            shape=(), minval=0, maxval=self._n_classes, dtype=tf.int32
        )
    ...
    def sample_from_class(self, cls_index: tf.Tensor) -> None:
        ...
        with tf.control_dependencies(
            [random_hshift, random_vshift, random_scale, random_fn_index]
        ):
            self._sample_update(
                hshift=random_hshift,
                vshift=random_vshift,
    ...
    def sample_from_function(self, cls_index: tf.Tensor, fn_index: tf.Tensor) -> None:
        random_hshift = self._rng.uniform(
            shape=(self._dims_params,),
            minval=self._hshift_min,
            maxval=self._hshift_max,
        ...
        with tf.control_dependencies([random_hshift, random_vshift, random_scale]):
            self._sample_update(
                hshift=random_hshift,
                vshift=random_vshift,
                scale=random_scale,
    ...
    def current_domain(self) -> tf.Tensor:
        ...
        with tf.control_dependencies([domain, hshift]):
            lower = tf.cast(
                tf.broadcast_to(domain[0], hshift.shape) - hshift, dtype=self._dtype
            )
            upper = tf.cast(
                tf.broadcast_to(domain[1], hshift.shape) - hshift, dtype=self._dtype
            )
            with tf.control_dependencies([lower, upper]):
                return tf.stack([lower, upper], axis=0)
