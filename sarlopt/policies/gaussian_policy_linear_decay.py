...
class GaussianPolicyLinearDecay(custom_gaussian_policy.CustomGaussianPolicy):
    ...
    def __init__(
        ...
        super().__init__(wrapped_policy, initial_scale, clip, name)
        self._scale_decay = tf.constant(
            (initial_scale - final_scale) / num_steps, dtype=self._action_spec.dtype
        )
        ...
        self._current_scale = tf.Variable(
            self._initial_scale, dtype=self._action_spec.dtype
        )
    ...
    def _action(
        ...
        self._current_scale.assign(
            tf.clip_by_value(
                self._current_scale - self._scale_decay,
                clip_value_min=self._final_scale,
                clip_value_max=self._initial_scale,
            )
        )

        self._noise_distribution.scale = (
            tf.ones(self._action_spec.shape, dtype=self._action_spec.dtype)
            * self._current_scale
        )
