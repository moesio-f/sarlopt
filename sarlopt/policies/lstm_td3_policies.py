...
class HistoryBuffer(object):
    def __init__(
        self, history_length, observation_spec, action_spec, name="HistoryBuffer"
    ):
        ...
        self._write_head = common.create_variable(
            initial_value=0, dtype=tf.int32, shape=(), name=name + "WriteHead"
        )

        self._written_size = common.create_variable(
            initial_value=0, dtype=tf.int32, shape=(), name=name + "WrittenSize"
        )
        ...
        self._lambda_read_full = lambda: tf.concat(
            [
                tf.range(self._write_head, self._written_size, dtype=tf.int32),
                tf.range(self._0, self._write_head, dtype=tf.int32),
            ],
            axis=-1,
        )
        self._lambda_read_otherwise = lambda: tf.range(
            self._0, self._written_size, dtype=tf.int32
        )
    ...
    def add(self, observation, action):
        ...
        self._written_size.assign_add(
            tf.cond(
                tf.less(self._written_size, self._hist_len),
                lambda: self._1,
                lambda: self._0,
            )
        )
        self._write_head.assign(tf.math.floormod(self._write_head + 1, self._hist_len))
        ...
        paddings = tf.cast(
            tf.reshape(
                tf.concat(
                    [
                        tf.stack(
                            [self._hist_len - self._written_size, self._0], axis=0
                        ),
                        tf.stack([self._0, self._0], axis=0),
                    ],
                    axis=0,
                ),
                [2, 2],
...
class LSTMTD3ActorPolicy(tf_policy.TFPolicy):
    ...
    def actor_network(self, copy=True) -> LSTMTD3ActorNetwork:
        ...
        if copy:
            actor_network = common.maybe_copy_target_network_with_checks(
                actor_network, None, name="ActorNetwork"
            )
    ...
    def _distribution(self, time_step, policy_state):
        ...
        network_inputs = LSTMTD3InputActor(history=history, observations=observation)

        actions, policy_state = self._actor_network(
            network_inputs, step_type=step_type, network_state=policy_state
        )

        self._memory.add(
            tf.squeeze(observation, axis=[0]), tf.squeeze(actions, axis=[0])
        )
        ...
        distributions = tf.nest.map_structure(_to_distribution, actions)
...
class LSTMTD3GaussianPolicy(tf_policy.TFPolicy):
    ...
    def _action(self, time_step, policy_state, seed=None):
        ...
        def _add_noise(action, distribution):
            ...
            if self._clip and isinstance(
                self.action_spec, tensor_spec.BoundedTensorSpec
            ):
                return common.clip_to_spec(action, self.action_spec)

            return noisy_action

        actions = tf.nest.map_structure(_add_noise, actions, self._noise_distribution)
