...
class UniformFunctionDistributionTest(tf.test.TestCase):
    ...
    def test_first_sample(self):
        ...
        expected_fx = (
            tf.multiply(
                self.functions[1][1](
                    x + tf.constant([3.3239317, 4.938116], dtype=self.dtype)
                ),
                0.8840528,
            )
            + 1.7433386
    ...
    def test_multiple_samples(self):
        ...
        expected_fx = (
            tf.multiply(
                self.functions[0][0](
                    x + tf.constant([3.9939976, -9.624966], dtype=self.dtype)
                ),
                1.8881679,
            )
            - 1.544632
        ...
        expected_fx = (
            tf.multiply(
                self.functions[2][0](
                    x + tf.constant([-9.09575, -3.2940674], dtype=self.dtype)
                ),
                1.4871273,
            )
            - 0.48787737
        ...
        expected_fx = (
            tf.multiply(
                self.functions[0][0](
                    x + tf.constant([6.7230206, -9.136286], dtype=self.dtype)
                ),
                1.2844284,
            )
            + 0.5755615
    ...
    def test_class_samples(self):
        ...
        self.assertEqual(
            tf.math.is_nan(expected_fx), tf.math.is_nan(fn_distribution(x))
        )
        ...
        expected_fx = (
            tf.multiply(
                self.functions[1][1](
                    x + tf.constant([-3.3616877, -8.070612], dtype=self.dtype)
                ),
                0.883859,
            )
            + 0.805696
    ...
    def test_function_samples(self):
        ...
        fn_distribution.sample_from_function(
        ...
        expected_fx = (
            tf.multiply(
                self.functions[1][1](
                    x + tf.constant([4.6392727, 5.350893], dtype=self.dtype)
                ),
                1.1110494,
            )
            - 1.81915
