...
def evaluate_agent(
    eval_env: tf_environment.TFEnvironment,
    ...
    eval_metrics = [
        tf_custom_metrics.ConvergenceMultiMetric(
            trajectory_size=steps + 1, function=function, buffer_size=episodes
        )
    ]

    eval_driver = dy_ed.DynamicEpisodeDriver(
        env=eval_env, policy=policy_eval, observers=eval_metrics, num_episodes=episodes
    )
    eval_driver.run = common.function(eval_driver.run)

    results = eager_compute(eval_metrics, eval_driver)
    ...
    ax.set(
        ...
        title="{0} on {1} ({2} Dims)".format(algorithm_name, function.name, dims),
    )
    ...
    if save_to_file:
        filename = os.path.join(
            save_dir, "{0}-{1}D-{2}.png".format(function.name, dims, algorithm_name)
        )
...
def eager_compute(
    metrics, driver, train_step=None, summary_writer=None, summary_prefix=""
):
...
def evaluate_baselines(
    functions: typing.List[tff.TensorflowFunction], dims: int, steps=500, episodes=100
):
    ...
    for fun in functions:
        ...
        for ep in range(episodes):
            gd_pos = rng.uniform(size=(dims,), low=fun.domain.min, high=fun.domain.max)

            nag_pos = rng.uniform(size=(dims,), low=fun.domain.min, high=fun.domain.max)

            gd = GD(fun, pos=gd_pos, steps=steps)
            gd_bs.append(gd[0][-1])
            gd_bs_it.append(gd[1])

            nag = NAG(fun, pos=nag_pos, steps=steps)
            nag_bs.append(nag[0][-1])
            nag_bs_it.append(nag[1])

        data_gd = BaselineEvalData(
            "GD",
            fun.name,
...
def GD(
    ...
    for t in range(steps):
        ...
        pos = tf.clip_by_value(
            pos - tf.multiply(grads, lr),
...
def NAG(
    ...
    for t in range(steps):
        ...
        pos = tf.clip_by_value(
            pos + velocity, clip_value_min=domain.min, clip_value_max=domain.max
        )
...
def plot_convergence(functions: typing.List[str], show=False, dpi=300, style=None):
    if style is None:
        style = ["-", "-", "-", "-", "--", "--"]
