...
def evaluate_policy(
    ...
    network = common.maybe_copy_target_network_with_checks(
        policy.actor_network(), name="ActorPolicy"
    )
    ...
    def log_processing(g: tf.Tensor):
        ...
        return tf.cond(
            pred=tf.greater_equal(abs_g, tf.math.exp(-p)),
    ...
    def sign_processing(g: tf.Tensor):
        p = tf.constant(10, dtype=tf.float32)
        return tf.cond(
            pred=tf.greater_equal(tf.abs(g), tf.math.exp(-p)),
    ...
    for _ in range(history_length):
        history_obs.append(
            tf.zeros(shape=policy.time_step_spec.observation.shape, dtype=tf.float32)
        )
        history_act.append(tf.zeros(shape=policy.action_spec.shape, dtype=tf.float32))
    ...
    for ep in range(episodes):
        ...
        x = rng.uniform(
            shape=(dims,), minval=domain[0], maxval=domain[1], dtype=tf.float32
        )
        best_fx = function(x)
        for step in range(steps):
            ...
            action, _ = network(
                LSTMTD3InputActor(
                    history=history, observations=tf.expand_dims(observation, axis=0)
                ),
                None,
                training=False,
            )
            ...
            x = tf.clip_by_value(
                x + action, clip_value_min=domain[0], clip_value_max=domain[1]
            )
...
def train_lstm_td3(
    functions: typing.List[fn_distributions.FunctionList],
    ...
    agent_dir = training_utils.create_agent_dir_str(
        algorithm_name, "distribution", dims
    )
    ...
    log_dir, log_eval_dir, log_train_dir = training_utils.create_logs_dir(agent_dir)
    ...
    print(
        "Initializing replay buffer by collecting experience for {0} "
        "episodes with a collect policy.".format(initial_collect_episodes)
    )
    initial_collect_driver.run()

    replay_buffer.get_next(sample_batch_size=batch_size, num_steps=history_length + 2)
    ...
    tf.summary.text(
        "Hyperparameters", training_utils.json_pretty_string(hp_dict), step=0
    )
    ...
    for ep in range(training_episodes):
        ...
        if ep % eval_interval == 0:
            result = evaluate_policy(
                policy=agent.policy,
                function=tff.Sphere(tff.core.Domain(-5.12, 5.12)),
                dims=dims,
                steps=eval_steps,
...
if __name__ == "__main__":
    train_lstm_td3(
        functions=[[tff.Sphere(tff.core.Domain(-5.12, 5.12))]],
