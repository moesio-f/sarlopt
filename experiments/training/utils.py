...
def load_lstm_td3_policy(policy_dir: str) -> lstm_td3_policies.LSTMTD3ActorPolicy:
    # Loading specs.
    input_spec = tensor_spec.from_pbtxt_file(
        os.path.join(policy_dir, "input_spec.pbtxt")
    )
    input_spec = lstm_td3_actor_network.LSTMTD3InputActor(*input_spec)
    output_spec = tensor_spec.from_pbtxt_file(
        os.path.join(policy_dir, "output_spec.pbtxt")
    )
    action_spec = tensor_spec.from_pbtxt_file(
        os.path.join(policy_dir, "action_spec.pbtxt")
    )
    ts_spec = tensor_spec.from_pbtxt_file(
        os.path.join(policy_dir, "time_step_spec.pbtxt")
    )
...
def save_policy(agent_dir: str, policy: tf_policy.TFPolicy, **kwargs):
    output_dir = os.path.join(agent_dir, "policy")
    tf_io.gfile.makedirs(output_dir)

    if isinstance(policy, lstm_td3_policies.LSTMTD3ActorPolicy) or isinstance(
        policy, lstm_td3_policies.LSTMTD3GaussianPolicy
    ):
        ...
        tensor_spec.to_pbtxt_file(
            os.path.join(output_dir, "input_spec.pbtxt"), input_spec
        )
        tensor_spec.to_pbtxt_file(
            os.path.join(output_dir, "output_spec.pbtxt"), output_spec
        )
        tensor_spec.to_pbtxt_file(
            os.path.join(output_dir, "action_spec.pbtxt"), policy.action_spec
        )
        tensor_spec.to_pbtxt_file(
            os.path.join(output_dir, "time_step_spec.pbtxt"), policy.time_step_spec
        )
...
def save_specs(agent_dir: str, specs_dict: typing.Dict):
    with open(os.path.join(agent_dir, "specs.json"), "w") as specs_file:
        json.dump(specs_dict, specs_file, indent=True)


def create_agent_dir_str(algorithm_name: str, info_str: str, dims: int) -> str:
    str_dims = str(dims)
...
def create_agent_dir(algorithm_name: str, function: core.Function, dims: int) -> str:
