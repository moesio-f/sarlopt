...
from tf_agents.environments import parallel_py_environment, tf_py_environment, wrappers
...
from sarlopt.environments import py_function_env as py_func_env
from sarlopt.metrics import tf_custom_metrics
from sarlopt.typing.types import LayerParam


def train_ppo(
    function: functions_core.Function,
    dims: int,
    ...
    agent_dir = training_utils.create_agent_dir(algorithm_name, function, dims)
    ...
    train_metrics = [
        ...
        tf_metrics.MaxReturnMetric(batch_size=num_parallel_environments),
    ]

    eval_metrics = [
        tf_metrics.AverageReturnMetric(buffer_size=eval_episodes),
    ...
    actor_net = actor_distribution_network.ActorDistributionNetwork(
        obs_spec, act_spec, fc_layer_params=actor_layers
    )
    ...
    value_net = value_network.ValueNetwork(
        obs_spec, fc_layer_params=value_layers, activation_fn=value_activation_fn
    )
    ...
    driver = dy_ed.DynamicEpisodeDriver(
        tf_env_training, agent.collect_policy, observers=observers_train, num_episodes=1
    )
    ...
    def compute_eval_metrics():
        return eval_utils.eager_compute(
            eval_metrics,
            eval_driver,
